...
每实例恰好一个 worker 独占连接、TLS、会话和工作 socket。create 创建一个空闲任务；stopped 期间该任务阻塞在四项有界命令队列，不做 I/O。IDF 使用 12288 字节任务栈、优先级 5、静态 TCB/队列/锁，以及一项二值完成信号；资源都归实例。栈余量必须由实板测量，不能用该预算声称通过。
