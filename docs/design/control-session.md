...
固定 ESP-IDF v6.1 / C3 的上一候选编译尺寸为会话对象 17848 字节（含 264 字节 AEAD reader）、Yamux 5552 字节；登录时还需 4096 字节握手接收区。上一轮官方 FRPS QEMU 容量探针在 TLS OPEN 后证实单次 5552 字节 Yamux 申请失败。当前候选把四条流的接收 ring 从 mux 对象中拆出，mux 编译尺寸降至 1488 字节：打开流时分别申请 1024 字节，释放流或销毁会话时清零释放。[同输入 C3 QEMU 复测](../operations/c3-yamux-stream-ring-capacity.md)已越过该旧失败点并进入 `AUTHENTICATING`，随后固定 SDK OpenETH 的 1522 字节 RX buffer 申请持续失败；未完成 Login、注册或 Pong。控制流从登录开始存活，工作流只在收到工作请求时取得 ring；降低单笔连续申请不保证后续总量足够。登录后释放握手区；合法 AEAD 长度头只确定容量，直到对应密文字节抵达才逐块申请最多十六个不超过 4096 字节的块。64 KiB 明文仍需合计 65536 字节动态块，tag 留在 reader 内。会话保留 1056 字节 AEAD 发送区、阶段复用的握手/控制区和三个工作槽。任何分配失败、认证失败、取消或销毁均清零释放已持有块；正常消费完成也逐块清零释放。另需 TLS 对象、SDK 内部内存、cJSON 临时分配和工作连接对象。编译尺寸不是运行峰值或 MCU 泄漏结论；证据与 P6-03 未验边界见 [连续内存检查点](../operations/p6-frp-chunked-aead.md)及[逐块分配审计](../operations/p6-frp-lazy-aead-memory-audit.md)。
