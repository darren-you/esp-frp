...
ESP-IDF v6.1 / C3 的会话分为 41264 字节对象、20912 字节 Yamux 和 65552 字节 AEAD 接收区，合计 127728 字节（不含 allocator 元数据），保持原有完整记录容量。会话保留 4 KiB AEAD 发送区、四条 Yamux ring、握手/控制暂存和三个工作槽。实板首次测得 Wi-Fi 在线时最大连续块只有 114688 字节，因此禁止再要求一个 127728 字节的连续分配。首次两块分配在真实 TLS 后仍因堆区分布失败，因此最终采用三块分配。任一分配失败都回滚；正常销毁和握手配置拒绝均清零三块内存。另需 TLS 对象、SDK 内部内存、cJSON 临时分配和每条连接 56 字节的对象。编译尺寸不是运行峰值或 MCU 泄漏结论；真实 SDK 网络、worker、双业务流及 Base/MQTT 组合仍待实板验证。
