...
接入工作流后，ESP-IDF v6.1 / C3 的会话对象为 127728 字节，包含 65552 字节 AEAD 接收区、4 KiB AEAD 发送区、四条 Yamux ring、握手/控制暂存和三个工作槽。另需 TLS 对象、SDK 内部内存、cJSON 临时分配和每条连接 56 字节的对象。编译尺寸不是运行峰值或 MCU 泄漏结论；真实 SDK 网络、worker、双业务流及 Base/MQTT 组合仍待实板验证。
