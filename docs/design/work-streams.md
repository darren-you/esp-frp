...
全组共享 4096 字节握手 payload 区，每槽保留各 1024 字节的两方向暂存。最多一条 SENDING/WAITING 流拥有共享区，活动流的关闭不能清除另一条等待流的部分握手；StartWorkConn parser 只消费一帧，后缀立即归业务方向，不能丢弃或再次解析。握手完成、握手失败或全组取消时清空 JSON 工作区。每次仅搬运接受的前缀，未消费字节保留到下一步，输出成功只意味着相应层接管了数据。
...
C3 当前会话分配为 24848 字节对象、20912 字节 Yamux 和 65552 字节 AEAD 接收区，合计 111312 字节；每条连接对象 56 字节，另需 TLS、lwIP、cJSON、allocator 元数据和任务栈。单份工作 JSON 节省 8192 字节，再按登录/控制互斥生命周期节省 8224 字节，两步均不削减接收消息上限。正式锁定 SDK 的独立 C3 样例大载荷双流已通过；完整 MCU 峰值、故障和 Base/MQTT 组合验收状态见 [问题记录](../issues/c3-loopback-memory-pressure.md)，不能由 host 测试推定通过。
