...
C3 当前会话分配为 41264 字节对象、20912 字节 Yamux 和 65552 字节 AEAD 接收区，合计 127728 字节；每条连接对象 56 字节，另需 TLS、lwIP、cJSON、allocator 元数据和任务栈。仅编译通过，尚未验证 MCU 堆峰值、内存不足时的清理、完整 worker/退避与 Base/MQTT 组合；host 测试不替代这些完成条件。
