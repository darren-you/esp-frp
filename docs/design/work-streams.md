...
C3 当前会话对象为 17688 字节、Yamux 为 5552 字节、AEAD 接收区为 65552 字节，常驻三项合计 88792 字节；工作握手开始接收时另分配 4096 字节，每条连接对象 56 字节，另需 TLS、lwIP、cJSON、allocator 元数据和任务栈。上述对象尺寸来自锁定 SDK 的 C3 编译调试信息。按需分配后，8 KiB worker 的同板十轮双流压力最低 heap 为 48296 字节；再将 worker 调至 6 KiB 的同类十轮压力最低 heap 为 60504 字节、worker 最低栈余量为 3104 字节。两轮条件不同，不能将最低 heap 差值全部归因于栈调整。早期通过单份工作 JSON 与登录/控制存储复用节省内存，随后将搬运块收敛到 1 KiB，均不削减 4 KiB JSON、64 KiB AEAD 或 256 KiB 协议信用。独立 C3 样例的 `work-tail-fin` 已通过，`work-local-fin` 正在重测，预备流、慢流和部分握手并存的实板场景尚未验收；完整 MCU 组合峰值、故障和 Base/MQTT 组合验收状态见 [问题记录](../issues/c3-loopback-memory-pressure.md)，不能由单项压力测试推定通过。
